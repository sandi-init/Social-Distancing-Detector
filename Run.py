...
while True:
	...
	for (i, (prob, bbox, centroid,n)) in enumerate(results):
		...
		tex="person:{:.4f}".format(prob)
		cv2.putText(frame, tex, (startX-2,startY-2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
	...
	text2="safe people count :{}".format(n-(len(serious)+len(abnormal)))
	cv2.putText(frame,text2,(10,frame.shape[0]),cv2.FONT_HERSHEY_SIMPLEX,0.70,(0,255,0),2)
...
#------------------------------Alert function----------------------------------#
	...
	if len(serious) >= config.Threshold:
		...
		if config.ALERT:
			...
			m=Mailer()
			m.send(config.MAIL,len(serious))
